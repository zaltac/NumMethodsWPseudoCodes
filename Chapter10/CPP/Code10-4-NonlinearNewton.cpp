...
void NONLINEAR_NEWTON(int M, const std::vector<double>& x, std::vector<double>& yo, std::vector<double>& y, double tol, const std::vector<double>& alpha, const std::vector<double>& beta, const std::vector<double>& gamma, int maxit) {
// ==================================================================================
//  CODE10.4-NONLINEAR_NEWTON.CPP. A C++ module implementing Pseudocode 10.4.                      
// 
//  NUMERICAL METHODS FOR SCIENTISTS AND ENGINEERS: WITH PSEUDOCODES
//  First Edition. (c) By Zekeriya ALTAÇ (2024).
//  ISBN: 978-1-032-75474-1 (hbk)
...
//  REVISION DATE :: 03/10/2025                                                                
// ==================================================================================
    std::vector<double> A(M), B(M), D(M), C(M);
    double h = x[1] - x[0];
    double hsqr = h * h;
    double bb2h = 0.5 / h;
    double hb2 = 0.5 * h;
    ...
    int bc_left = static_cast<int>(std::abs(alpha[0]));
    int bc_rigt = static_cast<int>(std::abs(alpha[1]));
...
int main() {
// ==============================================================================
//  The main program to test the module NONLINEAR_NEWTON.CPP
// ==============================================================================
    const int nmax = 201;
    int n;
    ...
    std::cin >> n;
    if (n > nmax) {
        std::cout << "Max grid size is " << nmax << std::endl;
        std::cout << "Increase NMAX, and then try it again." << std::endl;
        return 1;
    }
    ...
    double h = (xb - xa) / static_cast<double>(n);

    std::vector<double> x(neq), y(neq), yo(neq), alpha(2), beta(2), gamma(2);

    for (int i = 0; i < neq; ++i) {
        x[i] = xa + static_cast<double>(i) * h;
    }
    ...
    std::cout << std::endl << "        x        Exact     N.Approx   Abs Error" << std::endl;
    for (int i = 0; i < neq; ++i) {
        double aerr = std::abs(Exact(x[i]) - y[i]);
        std::cout << std::setw(10) << std::fixed << std::setprecision(5) << x[i]
                  << std::setw(12) << std::fixed << std::setprecision(7) << Exact(x[i])
                  << std::setw(12) << std::fixed << std::setprecision(7) << y[i]
                  << std::setw(15) << std::scientific << aerr << std::endl;
    }

    std::cout << std::endl << "*** D O N E ***" << std::endl;
